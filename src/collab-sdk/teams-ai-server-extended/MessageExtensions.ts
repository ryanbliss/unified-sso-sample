...
import { Query, TurnState } from "@microsoft/teams-ai";
...
export class MessageExtensions<TState extends TurnState> {
  ...
  public botMessagePreviewEdit(
    ...
    handler: (
      ...
      MessagingExtensionResult | TaskModuleTaskInfo | string | null | undefined
    >
  ): Application<TState> {
    ...
    (Array.isArray(commandId) ? commandId : [commandId]).forEach((cid) => {
      ...
      this._app.addRoute(
        ...
        async (context, state) => {
          ...
          const meQuery: MessagingExtensionQuery =
          ...
          (meQuery.parameters ?? []).forEach(
            (param: MessagingExtensionParameter) => {
              if (param.name) {
  ...
  public selectItem<TItem extends Record<string, any> = Record<string, any>>(
    ...
    const selector = (context: TurnContext) =>
      Promise.resolve(
        context?.activity?.type == ActivityTypes.Invoke &&
          context?.activity.name === SELECT_ITEM_INVOKE
      );
...
function createTaskSelector(
  ...
  if (typeof commandId == "function") {
    ...
    return (context: TurnContext) => {
      const isInvoke =
        context?.activity?.type == ActivityTypes.Invoke &&
        context?.activity?.name == invokeName;
      if (
        isInvoke &&
        ...
        return Promise.resolve(
          commandId.test(context.activity.value.commandId)
        );
      } else {
