...
import { TooManyStepsParameters } from "@microsoft/teams-ai/lib/types";
import * as actions from "@microsoft/teams-ai/lib/actions";
import {
  TurnState,
  Plan,
  Planner,
...
import { ActionEntry, ActionHandler } from "./actions";
...
export class AI<TState extends TurnState = TurnState> {
  private readonly _actions: Map<string, ActionEntry<TState>> =
    new Map();
  ...
  public constructor(options: AIOptions<TState>) {
    ...
    this.defaultAction(
      ...
      actions.sayCommand(this._options.enable_feedback_loop)
    );
   ...
   * @param {ActionHandler} handler The code to execute when the action's name is triggered.
   * @returns {this} The AI system instance for chaining purposes.
   */
  public action<TParameters extends Record<string, any> | undefined>(
    name: string | string[],
    handler: ActionHandler<TState, TParameters>
  ): this {
   ...
   * @param {string | string[]} name - Unique name of the action.
   * @template TParameters - Optional. The type of parameters that the action handler expects.
   * @param {ActionHandler<TState, TParameters>} handler - The code to execute when the action's name is triggered.
   ...
   * @param {IConversationContext} context Current turn context.
  ...
  public async run(
    ...
    try {
      ...
      let plan: Plan | undefined =
        step_count == 0
          ? await this._options.moderator.reviewInput(context, state)
          : undefined;

      // Generate plan if moderator did not return one as flag for input.
      if (!plan) {
        if (step_count == 0) {
          plan = await this._options.planner.beginTask(context, state, this as any);
        } else {
          plan = await this._options.planner.continueTask(context, state, this as any);
        }
      ...
      for (let i = 0; i < plan.commands.length; i++) {
        ...
        switch (cmd.type) {
            ...
            if (this._actions.has(action)) {
              ...
              output = await this._actions
                ...
                .handler(
                  context,
                  state,
                  { handler, ...(cmd as PredictedDoCommand) },
                  action
                );
          ...
          default:
            throw new Error(
              `AI.run(): unknown command of '${cmd.type}' predicted.`
            );
        }
