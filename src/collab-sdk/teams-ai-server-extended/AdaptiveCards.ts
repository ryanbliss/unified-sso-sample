...
import { TurnState, Query } from "@microsoft/teams-ai";
...
export class AdaptiveCards<TState extends TurnState> {
   ...
   * @param {(context: IConversationContext, state: TState, data: TData) => Promise<AdaptiveCard | string>} handler The code to execute when the action is triggered.
   * @param {IConversationContext} handler.context The current turn context.
            ...
            if (typeof result == "string") {
              ...
              if (
                ...
                actionExecuteResponseType !==
                  AdaptiveCardActionExecuteResponseType.NEW_MESSAGE_FOR_ALL
              ) {
                ...
                actionExecuteResponseType =
                  AdaptiveCardActionExecuteResponseType.REPLACE_FOR_ALL;
              }

              const activity = MessageFactory.attachment(
                CardFactory.adaptiveCard(result)
              );
              response = {
                statusCode: 200,
  ...
  public actionSubmit<TData = Record<string, any>>(
    verb: string | RegExp | RouteSelector | (string | RegExp | RouteSelector)[],
    handler: (context: IConversationContext, state: TState, data: TData) => Promise<void>
  ): Application<TState> {
    const filter =
      this._app.options.adaptiveCards?.actionSubmitFilter ??
      DEFAULT_ACTION_SUBMIT_FILTER;
    (Array.isArray(verb) ? verb : [verb]).forEach((v) => {
      ...
      this._app.addRoute(selector, async (context, state) => {
        ...
        if (
          ...
          throw new Error(
            `Unexpected AdaptiveCards.actionSubmit() triggered for activity type: ${a?.type}`
          );
        }
   ...
   * @param {IConversationContext} handler.context The current turn context for the handler callback.
    ...
    (Array.isArray(dataset) ? dataset : [dataset]).forEach((ds) => {
      ...
      this._app.addRoute(
        ...
        async (context, state) => {
          ...
          if (
            a?.type !== ActivityTypes.Invoke ||
            a?.name !== SEARCH_INVOKE_NAME
          ) {
            throw new Error(
              `Unexpected AdaptiveCards.search() triggered for activity type: ${a?.type}`
            );
          }
 ...
 */
function createActionExecuteSelector(
  ...
  if (typeof verb == "function") {
    ...
    return (context: IConversationContext) => {
      const a = context?.activity;
      const isInvoke =
        a?.type == ActivityTypes.Invoke &&
        a?.name === ACTION_INVOKE_NAME &&
 ...
 * @returns {RouteSelector} A function that matches the verb using a RegExp or attempts to match verb.
 */
function createActionSubmitSelector(
  verb: string | RegExp | RouteSelector,
  filter: string
): RouteSelector {
  if (typeof verb == "function") {
...
function createSearchSelector(
  dataset: string | RegExp | RouteSelector
): RouteSelector {
  if (typeof dataset == "function") {
    ...
    return (context: IConversationContext) => {
      const a = context?.activity;
      const isSearch =
        a?.type == ActivityTypes.Invoke && a?.name === SEARCH_INVOKE_NAME;
...
/**
 * @param {IConversationContext} context - The context of the current turn, providing information about the incoming activity and environment.
 * @param {AdaptiveCardInvokeResponse} response - The adaptive card invoke response to be sent.
 * @private
 */
async function sendInvokeResponse(
  context: IConversationContext,
