...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  console.log("/api/pubsub/note-change hub", hub);
  ...
  console.log("/api/pubsub/note-change secret", secret);
  const body = await req.json();
  console.log("/api/pubsub/note-change body", JSON.stringify(body, null, 4));
