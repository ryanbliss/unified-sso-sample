...
export async function POST(req: NextRequest): Promise<NextResponse> {
  console.log(
    "/api/pubsub/update-client-state origin",
    req.headers.get("WebHook-Request-Origin")
  );
  const url = new URL(req.url);
  const hub = url.searchParams.get("hub");
  console.log("/api/pubsub/update-client-state hub", hub);
  ...
  console.log("/api/pubsub/update-client-state secret", secret);
  const body = await req.json();
  console.log("/api/pubsub/update-client-state body", JSON.stringify(body, null, 4));
